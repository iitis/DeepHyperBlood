...
File with models used for training.
"""
# Torch
...
class LeeEtAl(nn.Module):
    ...
    def __init__(self, in_channels, n_classes):
        ...
        # WARNING! Conv3D, where third dimension is equal to in_channels is the same
        # thing as Conv2D
...
class LiEtAl(nn.Module):
    ...
    def forward(self, x):
        x = F.relu(self.conv1(x))
        x = F.relu(self.conv2(x))
        last_layer = x.view(-1, self.features_size)
        # x = self.dropout(x)
        x = self.fc(last_layer)
        return x
...
class MouEtAl(nn.Module):
    ...
    @staticmethod
    def weight_init(m):
        # All weight matrices in our RNN and bias vectors are initialized with a uniform
        # distribution, and the values of these weight matrices and bias vectors are
    ...
    def __init__(self, input_channels, n_classes):
        # The proposed network model uses a single recurrent layer that adopts our
        # modified GRUs of size 64 with sigmoid gate activation and PRetanh activation
        # functions for hidden representations
...
def train(net, optimizer, criterion, data_loader, epoch, scheduler=None,
          ...
          val_loader=None, supervision='full', rdir='results/',
          model_name=None, preprocessing=None, run=0):
    """
    ...
    save_epoch = epoch // 1 if epoch > 1 else 1

    losses = np.zeros(epoch*len(data_loader)+1)
    mean_losses = np.zeros(epoch*len(data_loader)+1)
    iter_ = 1
    loss_win, val_win = None, None
    # lists for validation accuracy and training losses
    val_accuracies = []
    avg_training_losses = []
    ...
    # save losses
    save_loss_accuracy(avg_training_losses,
                       val_accuracies,
                       net,
                       model_name,
                       data_loader.dataset.name,
...
def save_loss_accuracy(
        training_losses,
        val_accuracies,
        model, model_name, dataset_name, epoch, preprocessing, batch_size, run=0, rdir='results/'
        ):
    '''
    Function for saving training losses and validation accuracy
    from consecutive epochs
    '''
    model_dir = rdir# + camel_to_snake(str(model.__class__.__name__)) + "/" + dataset_name + "/"
    if not os.path.isdir(model_dir):
        os.makedirs(model_dir, exist_ok=True)
    filename = dataset_name + "_" + preprocessing + "_" + model_name + "_" + str(run) + \
        "_epoch_" + str(epoch) + "_batch_" + str(batch_size)

    # save training losses
    np.savetxt(model_dir + "training_losses_" + filename + ".csv",
               training_losses,
               delimiter='\n',
               fmt='%.6f')
    ...
    if val_accuracies:
        np.savetxt(model_dir + "val_accuracies_" + filename + ".csv",
                   val_accuracies,
                   delimiter='\n',
                   fmt='%.4f')


def save_model(model, model_name, dataset_name, epoch, metric, preprocessing, batch_size, run=0, rdir='results/'):
    model_dir = rdir# + camel_to_snake(str(model.__class__.__name__)) + "/" + dataset_name + "/"
    time_now = datetime.datetime.now()
    if not os.path.isdir(model_dir):
        os.makedirs(model_dir, exist_ok=True)
    if isinstance(model, torch.nn.Module):
        # filename = str(time_now.strftime("%Y-%m-%d_%H:%M:%S")) + "_epoch{epoch}_{metric:.2f}".format(epoch, metric)
        filename = 'checkpoint_' + dataset_name + "_" + preprocessing + \
            "_" + model_name + "_" + str(run) + "_epoch_" + str(epoch) + "_batch_" + str(batch_size)
